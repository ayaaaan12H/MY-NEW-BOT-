...
const login = require("fca-max");
const axios = require("axios");
const listPackage = JSON.parse(readFileSync('./package.json')).dependencies;
const listbuiltinModules = require("module").builtinModules;
console.log(chalk.bold.hex("#00ffff").bold("[ MAX LARKA (MAX) ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
console.log(global.getText('max', 'foundPathAppstate'))
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "appstate.json"));
    var appState = require(appStateFile);
    logger.loader(global.getText("max", "foundPathAppstate"))
}
catch { return logger.loader(global.getText("max", "notFoundPathAppstate"), "error") }
...
function checkBan(checkban) {
    ...
    logger(global.getText('max', 'checkListGban'), '[ GLOBAL BAN ]'), global.checkBan = !![];
    if (existsSync('/home/runner/.maxgban')) {
        ...
        logger(global.getText('max', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            _0x4244d8 = String(_0x4244d8);

            if (isNaN(_0x4244d8) || _0x4244d8.length < 6 || _0x4244d8.length > 6) 
                console.log(global.getText('max', 'keyNotSameFormat'));
            else return axios.get('https://raw.githubusercontent.com/marinakhan848marina-bot/fb-bot/main/listban.json').then(_0x2f978e => {
                const _0x360aa8 = _0x3d580d(String(_0x2f978e.data).replace(/\s+/g, '').toLowerCase());                
                if (_0x360aa8 !== _0x4244d8) return console.log(global.getText('max', 'codeInputExpired'));
                else {
                    const _0x1ac6d2 = {};
                    return _0x1ac6d2.recursive = !![], rm('/.maxgban', _0x1ac6d2), _0x2cd8f4.close(), 
                    logger(global.getText('max', 'unbanDeviceSuccess'), '[ GLOBAL BAN ]');
                }
            });
        });
        return;
    };
    return axios.get('https://raw.githubusercontent.com/marinakhan848marina-bot/fb-bot/main/listban.json').then(dataGban => {
        ...
        for (const adminID of admin) {
            if (!isNaN(adminID) && dataGban.data.hasOwnProperty(adminID)) {
                logger(global.getText('max','userBanned', dataGban.data[adminID]['dateAdded'], dataGban.data[adminID]['reason']), '[ GLOBAL BAN ]'), 
                mkdirSync(_0x4e5718 + ('/.maxgban'));
                if (_0x28e5ae == 'win32') execSync('attrib +H' + '+S' + _0x4e5718 + ('/.maxgban'));
                return process.exit(0);
            }
        }                                                                                                      
        if (dataGban.data.hasOwnProperty(checkban.getCurrentUserID())) {
            logger(global.getText('max', 'userBanned', dataGban.data[checkban.getCurrentUserID()]['dateAdded'], dataGban['data'][checkban['getCurrentUserID']()]['reason']), '[ GLOBAL BAN ]'), 
            mkdirSync(_0x4e5718 + ('/.maxgban'));
            if (_0x28e5ae == 'win32') 
                execSync('attrib +H +S ' + _0x4e5718 + ('/.maxgban'));
            return process.exit(0);
        }
        return axios.get('https://raw.githubusercontent.com/marinakhan848marina-bot/fb-bot/main/data.json').then(json => {
            logger(json.data[Math['floor'](Math['random']() * json.data.length)], '[ BROAD CAST ]');
        }), logger(global.getText('max','finishCheckListGban'), '[ GLOBAL BAN ]');
    }).catch(error => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        global.client.timeStart = new Date().getTime(),
            function () {
                const listCommand = readdirSync(global.client.mainPath + '/Max/commands').filter(command => command.endsWith('.js') && !command.includes('example') && !global.config.commandDisabled.includes(command));
                for (const command of listCommand) {
                    try {
                        var module = require(global.client.mainPath + '/Max/commands/' + command);
                        if (!module.config || !module.run || !module.config.commandCategory) throw new Error(global.getText('max', 'errorFormat'));
                        if (global.client.commands.has(module.config.name || '')) throw new Error(global.getText('max', 'nameExist'));
                        if (!module.languages || typeof module.languages != 'object' || Object.keys(module.languages).length == 0) logger.loader(global.getText('max', 'notFoundLanguage', module.config.name), 'warn');
                        if (module.config.dependencies && typeof module.config.dependencies == 'object') {
                            for (const reqDependencies in module.config.dependencies) {
                                ...
                                try {
                                    ...
                                    var check = false;
                                    var isError;
                                    logger.loader(global.getText('max', 'notFoundPackage', reqDependencies, module.config.name), 'warn');
                                    ...
                                    if (!check || isError) throw global.getText('max', 'cantInstallPackage', reqDependencies, module.config.name, isError);
                                }
                            }
                            logger.loader(global.getText('max', 'loadedPackage', module.config.name));
                        }
